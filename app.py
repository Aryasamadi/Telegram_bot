# VERSION 10.31.0-final — PART 1/3
# ✅ AI جهانی: هر مدل از هر شرکتی (بدون لیست سفید) + آبشار ابزار وب + fallback بدون ابزار
# ✅ تست مدل همیشه پاسخ می‌دهد (تایم‌اوت + try/except/finally) — هرگز روی «در حال تست» نمی‌ماند
# ✅ پیام توکن حذف نمی‌شود (طبق دستور مدیر)
# ✅ دکمه چت هوش مصنوعی کاربر حذف شده؛ پنل‌های سلامت حفظ شده‌اند
# ✅ دیپ‌لینک: اول تصویر + خلاصه کانال، سپس متن کامل
# ✅ منوها غنی‌تر (۲-۲ با ایموجی+کلمه) — قالب‌بندی محتوا دست‌نخورده
...
from aiogram.types import (Message, CallbackQuery, TelegramObject, InlineKeyboardMarkup, InlineKeyboardButton, BufferedInputFile)
...
WEBSCOUT_EMPTY_RETRY_MINUTES = max(10, int(os.getenv("WEBSCOUT_EMPTY_RETRY_MINUTES", "10")))
WEBSCOUT_HEARTBEAT_SECONDS = max(240, int(os.getenv("WEBSCOUT_HEARTBEAT_SECONDS", "240")))
WEBSCOUT_LOOP_SLEEP_SECONDS = max(15, int(os.getenv("WEBSCOUT_LOOP_SLEEP_SECONDS", "15")))
...
💾 ذخیرهٔ مطالب در پوشه‌های اختصاصی (فناوری، هوش مصنوعی، سایبری، آموزش)
🔍 جستجوی سریع داخل پوشه‌های خودت
📖 مطالعهٔ متن کامل مقالات با لینک «بیشتر بخوانید»
📞 ارتباط مستقیم با مدیریت فقط با دستور /man
    ...
    async def execute_batch(self, queries: List[Dict[str, Any]]) -> List[List[Dict[str, Any]]]:
        ...
        try:
            for query in queries:
                ...
                async with session.post(self.url, headers=self.headers, json=payload) as resp:
                    if resp.status != 200:
                        text = await resp.text(); raise Exception(f"Cloudflare D1 Batch API returned status {resp.status}: {text}")
                    data = await resp.json()
                    if not data.get("success"): raise Exception(f"D1 Batch Query failed: {data.get('errors')}")
                    result = data.get("result", [])
...
async def initialize_automation_database(db: D1Database):
    ...
    try: await db.execute("UPDATE ai_providers SET enabled=0, status='invalid', last_error='Environment Default disabled by managed-provider mode' WHERE name='Environment Default'")
    except Exception: pass
...
# ============================================================
# زیرساخت جهانی AI — هر مدل از هر شرکتی (FIX کامل)
# ============================================================
class AIProviderManager:
    def __init__(self, db: D1Database, bot: Optional[Bot] = None):
        self.db = db; self.bot = bot; self._session: Optional[aiohttp.ClientSession] = None
    ...
    @staticmethod
    def _extract_content(protocol: str, data) -> str:
    ...
    @staticmethod
    def _empty_response_reason(protocol: str, data) -> str:
    ...
    @staticmethod
    def _usage_tokens(protocol: str, usage) -> int:
    ...
    def classify_error(msg: str) -> str:
        ...
        if any(x in m for x in ("402", "404", "model_not_found", "does not exist", "401", "403", "authentication", "invalid api", "outstanding_balance")): return "invalid"
        return "temporary"
    @staticmethod
    def _is_tool_error(msg: str) -> bool:
        m = msg.lower()
    ...
    def _webscout_strategies(self, protocol: str, base: str) -> List[dict]:
        b = (base or "").lower(); out = []
        if protocol == "gemini":
    ...
    async def test_provider_values(self, base_url, api_key, model):
        ...
        for proto, endpoint in candidates:
            ...
            try:
                ...
                async with self._session.post(endpoint, headers=headers, json=payload, timeout=aiohttp.ClientTimeout(total=25)) as resp:
                    raw = await resp.text(); latency = int((time.perf_counter() - started) * 1000)
                    try: data = json.loads(raw)
                    except Exception: data = {}
                    if resp.status == 200 and self._extract_content(proto, data):
                        return {"ok": True, "latency_ms": latency, "preview": self._extract_content(proto, data).strip()[:120], "protocol": proto, "endpoint": endpoint}
                    if proto == "openai" and resp.status in {400, 422}:
                        async with self._session.post(endpoint, headers=headers, json={**payload, "temperature": 0, "max_tokens": 32}, timeout=aiohttp.ClientTimeout(total=25)) as r2:
                            raw2 = await r2.text()
                            try: d2 = json.loads(raw2)
                            except Exception: d2 = {}
                            if r2.status == 200 and self._extract_content("openai", d2):
                                return {"ok": True, "latency_ms": latency, "preview": self._extract_content("openai", d2).strip()[:120], "protocol": "openai", "endpoint": endpoint}
                    diagnostics.append(f"{proto} HTTP {resp.status}: {raw[:500]}")
            except Exception as e:
                diagnostics.append(f"{proto}: {str(e)[:400]}")
    ...
    async def call(self, messages, temperature=0.2, max_tokens=2500, purpose="generic", persist_health=True):
        ...
        for p in providers:
            ...
            try:
                ...
                if persist_health:
                    await self.db.execute("UPDATE ai_providers SET status='healthy',last_error=NULL,cooldown_until=NULL,last_checked_at=?,last_latency_ms=?,consecutive_failures=0,updated_at=? WHERE id=?", [datetime.now(timezone.utc).isoformat(), latency, datetime.now(timezone.utc).isoformat(), p["id"]])
                ...
                if persist_health:
                    ...
                    await self.db.execute("UPDATE ai_providers SET status=?,last_error=?,cooldown_until=?,consecutive_failures=COALESCE(consecutive_failures,0)+1,last_checked_at=?,updated_at=? WHERE id=?", ["invalid" if kind == "invalid" else "cooldown", msg[:1200], (datetime.now(timezone.utc) + timedelta(minutes=minutes)).isoformat(), datetime.now(timezone.utc).isoformat(), datetime.now(timezone.utc).isoformat(), p["id"]])
        final = ("همه مدل‌ها در cooldown یا نامعتبر هستند." if tried == 0 else "تمام مدل‌های قابل استفاده خطا دادند.") + " | " + " | ".join(errors)
        if purpose != "user_chat" and time.time() - self._last_final_notice > 1800 and self.bot and ADMIN_ID:
    ...
    async def webscout_call(self, url: str, scout_prompt: str, max_tokens: int = 9000):
        """WebScout جهانی: هر provider با هر پروتکل؛ آبشار ابزار وب سپس fallback بدون ابزار."""
        ...
        if not providers: return {"ok": False, "error": "هیچ WebScout فعالی در پنل AI وجود ندارد."}
        errors = []
        for p in providers:
            ...
            messages = [
                {"role": "system", "content": "You are the WebScout research engine. Use any enabled web tools to inspect the supplied URL; if no tools are available, answer strictly from retrieved knowledge."},
...
def make_deep_token(article_id: int) -> str:
    ...
    # ============================================================
# PART 2/3 — Sanitizer + Formatting (UNCHANGED) + Editorial + Cycle + Publish (FIXED)
# ============================================================
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag=="a":
            ...
            if href.startswith(("https://","http://","tg://")):
                self.out.append(f'<a href="{html.escape(href,quote=True)}">'); self.stack.append("a")
            return
...
def plain_len(value: str) -> int: return len(strip_html_text(value or ""))

def _format_technical_tokens(text: str) -> str:
    patterns=[r"\b(?:GPT-\d+(?:\.\d+)?|GPT-4o|LLM|API|JSON|Python|JavaScript|TypeScript|HTML|CSS|SQL|HTTP|HTTPS|OAuth|WebSocket|RAG|GPU|CPU|SDK)\b", r"\b(?:Generative AI|Machine Learning|Zero[- ]Day|Phishing|Ransomware)\b"]
    out=text
    for pat in patterns: out=re.sub(pat, lambda m: f"<code>{m.group(0)}</code>", out, flags=re.I)
    return out
...
def _protect_bidi_latin(text: str) -> str:
    ...
    parts=re.split(r"(<[^>]+>)", text, flags=re.I|re.S); out=[]
    for part in parts:
        if part.startswith("<") and part.endswith(">"): out.append(re.sub(r"[\u200e\u200f\u202a-\u202e\u2066-\u2069]","",part))
        else: out.append(re.sub(r"(?<![A-Za-z0-9])([A-Za-z][A-Za-z0-9@._+/#:-]{1,64})(?![A-Za-z0-9])", lambda m: "\u200e"+m.group(1)+"\u200e", part))
    return "".join(out)
...
def _split_readable_paragraphs(value: str, max_chars: int = 520) -> List[str]:
    ...
    for block in blocks:
        ...
        sentences=re.split(r"(?<=[.!?؟:])\s+", block); current=""
        for sent in sentences:
            sent=sent.strip()
            if not sent: continue
            candidate=(current+" "+sent).strip()
            if current and len(strip_html_text(candidate))>max_chars: out.append(current.strip()); current=sent
            else: current=candidate
        if current: out.append(current.strip())
    return out

def _remove_duplicate_title_from_body(title: str, value: str) -> str:
    text=_normalize_text_blocks(value or ""); title_plain=strip_html_text(title or "").strip()
    ...
    for block in blocks:
        ...
        looks=(sim>=0.72 or (title_plain.lower() in plain.lower() and len(plain)<=max(40,len(title_plain)*1.8)))
        if skipping and looks: continue
        skipping=False; kept.append(block)
    return "\n".join(kept)
...
def sanitize_resource_links(raw_links):
    ...
    for item in raw_links:
        ...
        url=normalize_url(str(item.get("url") or "")); label=strip_html_text(str(item.get("label") or item.get("title") or "")).strip()
...
def append_resource_links(article_html: str, resource_links, source_url: str = "") -> str:
    ...
    for x in sanitize_resource_links(resource_links):
        ...
        rendered.append(f'<a href="{html.escape(url,quote=True)}">{html.escape(label)}</a>'); break
...
def format_source_publication_date(raw: str) -> str:
    ...
    if not raw: return ""
    try:
        dt=datetime.fromisoformat(raw.replace("Z","+00:00")); return dt.strftime("%Y-%m-%d")
    except Exception:
...
def _persian_ratio(text: str) -> float:
    ...
    return len(re.findall(r"[؀-ۿ]",plain))/max(1,len(re.sub(r"\s+","",plain)))

def _latin_ratio(text: str) -> float:
    plain=strip_html_text(text or "")
    if not plain: return 0.0
    return len(re.findall(r"[A-Za-z]",plain))/max(1,len(re.sub(r"\s+","",plain)))
...
async def ai_editorial_process(ai: AIProviderManager,item:Dict[str,Any],source:Dict[str,Any],recent_titles:List[str],weights:Dict[str,float],manager_prompts:Optional[Dict[str,str]]=None):
    ...
    editorial_schema={"accept":True,"score":0,"global_relevance":0,"technology_relevance":0,"ai_relevance":0,"cyber_relevance":0,"education_relevance":0,"iran_relevance":0,"freshness":0,"reliability":0,"duplicate_risk":0,"category":"ai|tech|cyber|edu|general","why":"...","title":"...","channel_html":"...","article_html":"...","facts":["..."],"resource_links":[{"label":"...","url":"https://..."}]}
...
{json.dumps(editorial_schema, ensure_ascii=False)}"""
    ...
    if _needs_persian_rewrite(raw_title, raw_ch, raw_ar):
        ...
        if pobj:
            ...
            raw_ch=str(pobj.get("channel_html") or raw_ch); raw_ar=str(pobj.get("article_html") or raw_ar)
            obj["title"]=raw_title; obj["channel_html"]=raw_ch; obj["article_html"]=raw_ar; result=repaired
    title=raw_title
    ...
    dims={"global":float(obj.get("global_relevance",5) or 0),"technology":float(obj.get("technology_relevance",5) or 0),"ai":float(obj.get("ai_relevance",5) or 0),"cyber":float(obj.get("cyber_relevance",5) or 0),"education":float(obj.get("education_relevance",5) or 0),"iran":float(obj.get("iran_relevance",0) or 0),"freshness":float(obj.get("freshness",5) or 0),"source":max(0,min(10,float(source.get("trust_score") or 80)/10)),"novelty":10-max(0,min(10,float(obj.get("duplicate_risk",0) or 0)))}
...
async def get_manager_editorial_prompts(db: D1Database) -> Dict[str,str]:
    return {"channel": await get_setting(db,"editorial_prompt_channel","فقط محتوای فنی و واقعاً ارزشمند برای مخاطب فناوری و هوش مصنوعی را پوشش بده."),"article": await get_setting(db,"editorial_prompt_article","نسخه کامل باید فنی، غنی و مبتنی بر واقعیت‌های منبع باشد.")}
...
منبع: {source.get('name')} | دسته: {source.get('category')} | عنوان: {item.get('title')} | لینک: {item.get('url')} | تاریخ: {item.get('published_at')}
خلاصه/متن: {body} | شباهت با عناوین اخیر: {sim:.2f}
فقط JSON معتبر: accept, score, category, importance_reason, iran_relevance, freshness, reliability, duplicate_risk, event_date, why"""
    result=await ai.call([{"role":"system","content":"You are a strict editorial gate. Output JSON only."},{"role":"user","content":prompt}],0.1,900,"candidate_scoring")
    obj=parse_json_object(result.get("content",""))
    return {**obj,"ai":result} if obj else {"accept":False,"score":0,"reason":"AI returned invalid JSON","ai":result}
...
خروجی فقط JSON: title, channel_text, article_text, category, facts, image_note"""
    result=await ai.call([{"role":"system","content":"You are an expert Persian technology editor. Output JSON only."},{"role":"user","content":prompt}],0.35,4500,"content_generation")
    obj=parse_json_object(result.get("content",""))
    return {**obj,"ai":result} if obj else {"error":"invalid generation JSON","ai":result}
...
فقط JSON: ok, issues, confidence"""
    result=await ai.call([{"role":"system","content":"You are a strict fact-checking editor. Output JSON only."},{"role":"user","content":prompt}],0,1200,"content_verification")
    obj=parse_json_object(result.get("content",""))
...
async def add_source(db: D1Database, url: str, category: str = "tech", interval_minutes: Optional[int] = None, priority: int = 5) -> int:
    ...
    if not source_id: source_id=(await db.execute("SELECT id FROM sources WHERE url=?",[clean]))[0].get("id")
    invalidate_sources()
...
async def fetch_source_cycle(db: D1Database, source: Dict[str,Any], ai: AIProviderManager, progress=None, allow_old_test=False):
    ...
    # FIX: بررسی کانال قبل از هر پردازش سنگین
...
11) Return FALSE when no item satisfies all criteria; never return a weak substitute."""
    ...
    if insuff:
        ...
        await log_automation(db,'INFO','content_rejected',f"{source.get('name')} | {reason}"); return stats
    if len(strip_html_text(item['body'])) < 300:
        stats['diagnostics']=['WebScout research برای تولید محتوای غنی کوتاه بود']; return stats
    # FIX: جلوگیری از تکرار با source_url
    if await db.execute("SELECT id FROM articles WHERE source_url=? LIMIT 1",[item['url']]):
    ...
    if not manager_accepts_score(float(out.get('score',0) or 0),min_score):
        ...
        await log_automation(db,'INFO','content_rejected',f"{source.get('name')} | score={out.get('score')}"); return stats
    now=datetime.now(timezone.utc).isoformat()
...
async def get_runtime_bot_username(bot: Bot) -> str:
    ...
    try:
        me=await bot.get_me(); BOT_USERNAME_RUNTIME=me.username or BOT_USERNAME.lstrip("@")
    except Exception: BOT_USERNAME_RUNTIME=BOT_USERNAME.lstrip("@")
    ...
    for b in blocks:
        ...
        if plain_len(cand)<=limit: cur=cand
            ...
            if plain_len(b)<=limit: cur=b
                ...
                for i in range(0,len(plain),limit): chunks.append(html.escape(plain[i:i+limit],quote=False))
                cur=""
    if cur: chunks.append(cur)
    return chunks
...
async def recover_publication_queue(db: D1Database):
    try:
        await db.execute("UPDATE publication_queue SET status='queued', last_error='recovered' WHERE status='publishing'")
...
async def publish_next_article(db: D1Database, bot: Bot, force: bool=False) -> bool:
    ...
    async with PUBLISH_LOCK:
        if time.time()-LAST_RECOVER>600: await recover_publication_queue(db); LAST_RECOVER=time.time()
        if force:
            channel_id=await get_channel_id(db)
            ...
            count_rows=await db.execute("SELECT COUNT(*) c FROM articles WHERE status='published' AND COALESCE(published_at,created_at) >= ?",[tehran.replace(hour=0,minute=0,second=0,microsecond=0).astimezone(timezone.utc).isoformat()])
            if (count_rows[0].get("c",0) if count_rows else 0)>=int(await get_setting(db,"max_daily_posts",str(DEFAULT_MAX_DAILY_POSTS))): return False
        elif not await can_publish_now(db): return False
        now_iso=datetime.now(timezone.utc).isoformat()
        ...
        await db.execute("UPDATE publication_queue SET status='publishing',attempts=attempts+1,last_error=NULL WHERE id=? AND status='queued'",[queue_id])
        try:
            ...
            if not strip_html_text(channel_text): channel_text=f"<b>{html.escape(title_out[:200])}</b>\n{html.escape(strip_html_text(row.get('body') or '')[:400],quote=False)}"
            image_url=await resolve_article_image(db,row)
            caption=publication_caption(title_out,channel_text,deep_link)
            sent=None
            if image_url:
                try: sent=await bot.send_photo(chat_id=channel_id,photo=image_url,caption=caption,parse_mode="HTML")
                except Exception as img_error: await log_automation(db,"WARN","source_image_failed",f"article={article_id} {img_error}")
            if sent is None:
                # FIX: fallback متنی همیشه با لینک عمیق و برش امن
                try: sent=await bot.send_message(chat_id=channel_id,text=caption,parse_mode="HTML",disable_web_page_preview=True)
                except Exception: sent=await bot.send_message(chat_id=channel_id,text=strip_html_text(channel_text)[:3500]+"\n"+deep_link,disable_web_page_preview=True)
...
async def recheck_failed_providers(db,bot,manager):
    now=datetime.now(timezone.utc).isoformat()
    ...
    for p in rows:
        ...
        except Exception as e: await log_automation(db,"ERROR","provider_recheck_failed",f"provider={p.get('id')} {e}")

def source_is_due(s, now_dt):
    raw=s.get("next_check_at") or ""
    if not raw: return True
    try: return datetime.fromisoformat(raw.replace("Z","+00:00"))<=now_dt
    except Exception: return True

async def update_source_after_check(db, source, interval_minutes, error=None):
    now=datetime.now(timezone.utc)
    try:
        ...
        while True:
            ...
            try:
                ...
                if await get_setting(db,'automation_enabled','0')=='1':
                    if time.time()-last_recover>600: await recover_publication_queue(db); last_recover=time.time()
                    # FIX: انتشار مستقل از due بودن وب‌اسکات
                    if time.time()-last_publish_try>=PUBLISH_ATTEMPT_INTERVAL:
                        last_publish_try=time.time(); await publish_next_article(db,bot)
                    ...
                    if due:
                        ...
                        for src in ordered:
                            ...
                            try:
                                r=await fetch_source_cycle(db,src,ai); results.append(r)
                        ...
                        summary={'sources':len(results),'sources_checked':len(results),'processed':sum((r.get('processed',0) if isinstance(r,dict) else 0) for r in results),'accepted':sum((r.get('accepted',0) if isinstance(r,dict) else 0) for r in results),'rejected':sum((r.get('rejected',0) if isinstance(r,dict) else 0) for r in results),'errors':sum((r.get('errors',0) if isinstance(r,dict) else 0) for r in results),'queued':sum((r.get('queued',0) if isinstance(r,dict) else 0) for r in results),'published':False,'mode':'webscout'}
...
async def next_publication_estimate(db) -> Dict[str,Any]:
    now=datetime.now(timezone.utc)
    last_manual=await get_setting(db,"last_manual_channel_post_at","")
...
async def get_schedule_panel(db):
...
async def automation_report(db) -> str:
...
async def automation_overview(db) -> str:
    enabled=(await get_setting(db,"automation_enabled","0"))=="1"
    ...
    return ("📰 <b>اتوماسیون محتوا</b>\n"
            ...
            # ============================================================
# PART 3/3 — Keyboards(rich) + States + Middleware + Handlers + main()
# ============================================================
...
def get_main_menu(): return InlineKeyboardMarkup(inline_keyboard=[
    ...
    [InlineKeyboardButton(text="❓ راهنمای استفاده", callback_data="user_help")]])
...
def get_admin_search_pagination_kb(pid, index): return InlineKeyboardMarkup(inline_keyboard=[
    ...
    [InlineKeyboardButton(text="🗑 حذف", callback_data=f"adelete_{pid}"), InlineKeyboardButton(text="🔙 مدیریت", callback_data="admin_content")]])
...
class RateLimitMiddleware(BaseMiddleware):
    def __init__(self, admin_id): super().__init__(); self.admin_id=admin_id; self.m={}
    async def __call__(self, handler, event, data):
        uid=event.from_user.id if getattr(event,"from_user",None) else None
        if uid and uid!=self.admin_id:
            now=time.time()
            if now-self.m.get(uid,0.0)<1.0:
...
async def admin_ok(call):
    if call.from_user.id!=ADMIN_ID:
        ...
        return False
    return True
async def register_user_if_not_exists(db,uid): await db.execute("INSERT OR IGNORE INTO users(id,joined_at) VALUES(?,?)",[uid,datetime.now(timezone.utc).isoformat()])
async def send_post_content(bot,chat_id,post,reply_markup=None):
    text=post.get("text") or ""; fid=post.get("file_id"); mt=post.get("media_type")
    ...
    except Exception as e: logger.error("send post failed: %s",e); return None
async def send_article_content(bot,chat_id,article,reply_markup=None):
    ...
    src=normalize_url(article.get('source_url') or '')
    if src and 'منبع اصلی' not in strip_html_text(body): body=f"{body.rstrip()}\n<a href=\"{html.escape(src,quote=True)}\">منبع اصلی</a>"
    rel=relative_time_label(article.get('source_published_at') or article.get('published_at') or '')
    if rel!='زمان نامشخص': body=body.rstrip()+f"\n<i>⏱ {rel}</i>"
...
async def deliver_article_by_token(message,bot,db,token):
    ...
    if plain_len(intro)>700: intro=html.escape(strip_html_text(intro)[:700].rsplit(' ',1)[0]+"…",quote=False)
    rel=relative_time_label(article.get('source_published_at') or article.get('published_at') or '')
...
async def cmd_start(message,state,db,bot):
    ...
    await register_user_if_not_exists(db,uid); await state.set_state(BotStates.idle)
    sd=await state.get_data(); args=message.text.split()
    if len(args)>1:
        da=args[1]
    ...
    await message.answer(random.choice([f"سلام {name} عزیز! 👋 خیلی خوش اومدی. وقت کاوش تو دنیای تکنولوژیه! 🚀",f"درود {name}! 🌟 خوشحالیم که اینجایی. آماده‌ای برای مطالب جذاب؟ 📚",f"سلام {name} جان! 🤖 به پایگاه دانش ما خوش اومدی. بزن بریم که کلی مطلب خفن داریم! 🔥"])+"\nاز دکمه های پایین استفاده کنید👇🏻",reply_markup=menu)
...
async def cmd_reset_db(message,db):
    if message.from_user.id==ADMIN_ID:
        for t in ["users","posts","saves","votes","article_saves","article_votes","user_content_saves","user_content_votes","user_states","processed_updates","sources","source_items","articles","publication_queue","ai_providers","automation_settings","automation_logs","manual_channel_events","test_history"]: await db.execute(f"DROP TABLE IF EXISTS {t}")
...
async def intercept_global_commands(message,state,db):
    ...
    if text=="کاربر": await state.update_data(admin_mode="user"); await message.answer("✅ فاز کاربری فعال شد.",reply_markup=get_main_menu())
        ...
        joined=rows[0].get("joined_at") if rows else ""; role=rows[0].get("role") if rows else "user"
        try:
            jdt=datetime.fromisoformat(str(joined).replace('Z','+00:00'))
            if jdt.tzinfo is None: jdt=jdt.replace(tzinfo=timezone.utc)
            days=max(0,(datetime.now(timezone.utc)-jdt).days); date_line=jdt.astimezone(pytz.timezone("Asia/Tehran")).strftime("%Y/%m/%d")
        except Exception: days,date_line=0,"نامشخص"
        ...
        total_saves=sum(per.values()); likes=(await db.execute("SELECT COUNT(*) c FROM user_content_votes WHERE user_id=? AND vote_type='like'",[uid]))[0].get("c",0); dislikes=(await db.execute("SELECT COUNT(*) c FROM user_content_votes WHERE user_id=? AND vote_type='dislike'",[uid]))[0].get("c",0)
        await message.answer(f"👤 <b>پروفایل کاربری</b> · {html.escape(message.from_user.first_name or 'دوست عزیز')}\n🆔 شناسه: <code>{uid}</code>\n🔰 نقش: <b>{'مدیر 🌟' if role=='admin' else 'کاربر عادی 🟢'}</b>\n📅 تاریخ عضویت: <b>{date_line}</b>\n⏳ مدت همراهی: <b>{days} روز</b>\n💾 مجموع ذخیره‌ها: <b>{total_saves}</b>\n• 💻 فناوری: {per.get('tech',0)} · 🧠 هوش مصنوعی: {per.get('ai',0)}\n• 🔒 سایبری: {per.get('cyber',0)} · 📚 آموزش: {per.get('edu',0)}\n👍 لایک‌ها: <b>{likes}</b> · 👎 دیس‌لایک‌ها: <b>{dislikes}</b>",parse_mode="HTML",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="💾 مشاهده ذخیره‌ها",callback_data="user_saves"),InlineKeyboardButton(text="🏠 منوی اصلی",callback_data="user_home")]]))
    ...
    elif text=="⚙️ اتوماسیون محتوا" and uid==ADMIN_ID: await message.answer(await automation_overview(db),parse_mode="HTML",reply_markup=automation_menu_kb((await get_setting(db,"automation_enabled","0"))=="1"))
    elif text in ("➕ افزودن پست","📁 مدیریت محتوا","📊 آمار","📢 ارسال همگانی","⚙️ اتوماسیون محتوا"): await message.answer("⛔ شما دسترسی مدیریت ندارید.")
...
async def admin_stats(call,db):
    ...
    await call.message.edit_text(f"📊 <b>آمار کلی</b>\n👥 کاربران: {a.get('u',0)}\n📝 فعال: {a.get('p',0)}\n👁 بازدید: {a.get('v',0)}",parse_mode="HTML",reply_markup=get_admin_back_kb("admin_home")); await call.answer()
...
async def source_view(call,db):
    ...
    s=rows[0]
    kb=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔄 تست اکنون",callback_data=f"source_test_{sid}")],[InlineKeyboardButton(text="🔢 اولویت",callback_data=f"source_priority_{sid}"),InlineKeyboardButton(text="⏱ فاصله",callback_data=f"source_interval_{sid}")],[InlineKeyboardButton(text="⏸/▶️ وضعیت",callback_data=f"source_toggle_{sid}"),InlineKeyboardButton(text="🗑 حذف منبع",callback_data=f"source_delete_{sid}")],[InlineKeyboardButton(text="🔙 بازگشت به منابع",callback_data="auto_sources")]])
...
async def provider_model_input(message,state,db,bot):
    ...
    invalidate_providers()
    await message.answer(f"✅ مدل ذخیره شد.\n🤖 {html.escape(model)}\n⚡ {result.get('latency_ms',0)}ms",parse_mode="HTML",reply_markup=provider_list_kb(await db.execute("SELECT * FROM ai_providers ORDER BY priority ASC,id ASC")))
...
async def provider_view(call,db):
    ...
    kb=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✏️ ویرایش",callback_data=f"provider_edit_{pid}"),InlineKeyboardButton(text="🧪 تست",callback_data=f"provider_test_{pid}")],[InlineKeyboardButton(text="🔢 اولویت",callback_data=f"provider_priority_{pid}"),InlineKeyboardButton(text="⏸/▶️",callback_data=f"provider_toggle_{pid}")],[InlineKeyboardButton(text="🌐 WebScout: "+("روشن" if p.get("web_enabled") else "خاموش"),callback_data=f"provider_web_toggle_{pid}")],[InlineKeyboardButton(text="🗑 حذف مدل",callback_data=f"provider_delete_{pid}")],[InlineKeyboardButton(text="🔙 بازگشت به مدل‌ها",callback_data="auto_providers")]])
...
async def provider_test(call,db):
    ...
    await call.message.edit_text(("✅ موفق · "+str(r.get("latency_ms",0))+"ms") if r.get("ok") else "❌ "+html.escape(str(r.get("error",""))[:1000]),parse_mode="HTML",reply_markup=get_admin_back_kb(f"provider_view_{pid}"))
...
async def channel_test(call,db,bot):
    ...
    except Exception as e: await call.message.edit_text("❌ "+html.escape(str(e)[:800]),parse_mode="HTML",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 بازگشت",callback_data="auto_channel")]]))
    await call.answer()
...
async def quality_weights(call,db):
    ...
    for i in range(0,len(items),2):
        row=[]
        for k,lab in items[i:i+2]: row.append(InlineKeyboardButton(text=f"{lab}: {await get_setting(db,'weight_'+k,'10')}",callback_data="weight_"+k))
        rows.append(row)
...
async def admin_automation_setting_input(message,state,db,bot):
    ...
    try:
        ...
        elif key in {"min_hours_between_posts","min_post_gap_minutes"}: await set_setting(db,"min_post_gap_minutes",str(max(1,int(float(value)))))
    ...
    except Exception as e: await message.answer(f"❌ مقدار نامعتبر: {html.escape(str(e))}",reply_markup=get_admin_back_kb(parent))
...
# ---------- Content db / queue / articles / health ----------
...
async def render_automation_article(call,db,aid):
    ...
    if not rows: await call.answer("یافت نشد",show_alert=True); return
    a=rows[0]
    kb=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="👁 متن",callback_data=f"auto_art_view_{aid}")],[InlineKeyboardButton(text="✏️ عنوان",callback_data=f"auto_art_edit_title_{aid}"),InlineKeyboardButton(text="✏️ کانال",callback_data=f"auto_art_edit_channel_{aid}")],[InlineKeyboardButton(text="✏️ کامل",callback_data=f"auto_art_edit_body_{aid}"),InlineKeyboardButton(text="🗑 حذف",callback_data=f"auto_art_delete_{aid}")],[InlineKeyboardButton(text="🔙",callback_data="auto_queue" if a.get("qs")=="queued" else "auto_articles")]])
...
async def auto_health(call,db,bot):
    ...
    kb=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🤖 تست مدل‌ها",callback_data="health_test_ai"),InlineKeyboardButton(text="🌐 تست منابع",callback_data="health_test_source")],[InlineKeyboardButton(text="🧪 تولید بدون انتشار",callback_data="health_dry_run"),InlineKeyboardButton(text="▶️ چرخه واقعی",callback_data="health_run_cycle")],[InlineKeyboardButton(text="📢 تست انتشار",callback_data="health_test_publish")],[InlineKeyboardButton(text="🚦 وضعیت",callback_data="health_deployment"),InlineKeyboardButton(text="📜 لاگ",callback_data="health_logs")],[InlineKeyboardButton(text="🔄",callback_data="auto_health"),InlineKeyboardButton(text="🔙",callback_data="auto_back")]])
...
async def user_saves(call): await call.message.edit_text("💾 <b>ذخیره‌های من</b>\nیک پوشه انتخاب کن:",parse_mode="HTML",reply_markup=unified_saved_kb("all")); await call.answer()
...
async def user_profile(call,db):
    ...
    per={r["folder"]:r["c"] for r in await db.execute("SELECT folder,COUNT(*) c FROM user_content_saves WHERE user_id=? GROUP BY folder",[uid])}
    total_saves=sum(per.values()); likes=(await db.execute("SELECT COUNT(*) c FROM user_content_votes WHERE user_id=? AND vote_type='like'",[uid]))[0].get("c",0); dislikes=(await db.execute("SELECT COUNT(*) c FROM user_content_votes WHERE user_id=? AND vote_type='dislike'",[uid]))[0].get("c",0)
    await call.message.edit_text(f"👤 <b>پروفایل کاربری</b> · {html.escape(call.from_user.first_name or 'دوست عزیز')}\n🆔 شناسه: <code>{uid}</code>\n🔰 نقش: <b>{'مدیر 🌟' if role=='admin' else 'کاربر عادی 🟢'}</b>\n📅 تاریخ عضویت: <b>{date_line}</b>\n⏳ مدت همراهی: <b>{days} روز</b>\n💾 مجموع ذخیره‌ها: <b>{total_saves}</b>\n• 💻 فناوری: {per.get('tech',0)} · 🧠 هوش مصنوعی: {per.get('ai',0)}\n• 🔒 سایبری: {per.get('cyber',0)} · 📚 آموزش: {per.get('edu',0)}\n👍 لایک‌ها: <b>{likes}</b> · 👎 دیس‌لایک‌ها: <b>{dislikes}</b>",parse_mode="HTML",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="💾 مشاهده ذخیره‌ها",callback_data="user_saves"),InlineKeyboardButton(text="🏠 منوی اصلی",callback_data="user_home")]])); await call.answer()
...
async def process_f_search(call,state):
    ...
    cnt,ws=sd.get("search_count",0),sd.get("search_window_start",0)
    if now-ws>W: cnt,ws=0,0
    if cnt>=5: await call.answer("🛑 سهمیه جستجو تمام شده",show_alert=True); return
    await state.set_state(BotStates.user_search_folder); await state.update_data(folder=folder)
    await call.message.answer(f"🔍 کلمه موردنظر در پوشه {FOLDER_NAMES.get(folder,folder)} را بفرست:")
    await call.answer()
...
async def adm_view_all(call):
    if not await admin_ok(call): return
    await call.message.edit_text("📋 نمایش همه؟",parse_mode="HTML",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✅",callback_data="adm_view_all_confirm"),InlineKeyboardButton(text="❌",callback_data="adm_view_all_cancel")]])); await call.answer()
...
async def adm_view_all_confirm(call,state,db,bot):
    ...
    else: await call.message.edit_text("📭 محتوایی نیست.",reply_markup=get_content_management_kb())
    await call.answer()
...
async def adm_search_text(call,state):
    ...
    await call.message.edit_text("🔍 کلمه یا شماره پست:",parse_mode="HTML",reply_markup=get_exit_menu()); await call.answer()
...
async def process_admin_search_word(message,state,db,bot):
    ...
    if not ids: await message.answer("❌ پیدا نشد.",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔍 دوباره",callback_data="adm_search_text"),InlineKeyboardButton(text="🔙",callback_data="admin_content")]])); return
...
async def conf_broad_yes(call,state,db,bot):
    ...
    async def send(uid):
        try:
            ...
            elif m=="audio" and f: await bot.send_audio(uid,f,caption=t[:1020])
...
async def main():
    ...
    try:
        ...
        logger.info("Bot started (10.31.0-final).")
